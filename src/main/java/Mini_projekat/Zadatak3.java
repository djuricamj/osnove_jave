...
//Napomena: Smatrajte da se broj placeholdera podudara sa brojem elemenata u nizu.
        ...
        String sql = "INSERT INTO Customers (CustomerName, ContactName, Address, City, PostalCode, Country) VALUES ('%0', '%1', '%2', '%3', '%4', '%5');";
        ...
        sql =  sql.replace("%0", values.get(0));
        sql =  sql.replace("%1", values.get(1));
        sql = sql.replace("%2", values.get(2));
        sql =  sql.replace("%3", values.get(3));
        sql = sql.replace("%4", values.get(4));
        sql = sql.replace("%5", values.get(5));
